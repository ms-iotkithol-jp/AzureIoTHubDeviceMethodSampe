        ...
        public static int Main(string[] args)
        {
            ...
            if (File.Exists(configFileName))
            {
                using (var reader = new StreamReader(File.OpenRead(configFileName)))
                {
                    ...
                    if (!(configJson["device-connection-string"] is null))
                    {
                        s_deviceConnectionString = configJson["device-connection-string"];
                    }
                    if (!(configJson["target-service-hostname"] is null))
                    {
                        s_hostName = configJson["target-service-hostname"];
                    }
                    if (!(configJson["target-service-port"] is null))
                    {
                        s_port = configJson["target-service-port"];
                    }
                }
            }
            ...
            int argHostIndex = 1;
            int argPortIndex = 2;
            if (string.IsNullOrEmpty(s_deviceConnectionString) && args.Length == 1)
            {
            ...
            if (string.IsNullOrEmpty(s_hostName) && args.Length == 2)
            {
                s_hostName = args[argHostIndex];
            }
